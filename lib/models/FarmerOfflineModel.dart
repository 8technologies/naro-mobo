...
class FarmerOfflineModel {
  static String tableName = "offline_farmers_model_1";
  int id = 0;
  ...
  String ready_for_upload = "No";
  String error_message = "";
  String local_id = "";

  static fromJson(dynamic m) {
    FarmerOfflineModel obj = new FarmerOfflineModel();
    if (m == null) {
      return obj;
    }
    ...
    obj.bank_id = Utils.to_str(m['bank_id'], '');
    obj.bank_text = Utils.to_str(m['bank_text'], '');
    obj.other_livestock_count = Utils.to_str(m['other_livestock_count'], '');
    obj.poultry_count = Utils.to_str(m['poultry_count'], '');
    obj.sheep_count = Utils.to_str(m['sheep_count'], '');
    obj.goat_count = Utils.to_str(m['goat_count'], '');
    obj.cattle_count = Utils.to_str(m['cattle_count'], '');
    obj.bank_account_number = Utils.to_str(m['bank_account_number'], '');
    obj.has_receive_loan = Utils.to_str(m['has_receive_loan'], '');

    obj.ready_for_upload = Utils.to_str(m['ready_for_upload'], '');
  ...
  static Future<List<FarmerOfflineModel>> getLocalData(
  ...
  static Future<List<FarmerOfflineModel>> get_items(
      {String where = '1'}) async {
  ...
  save() async {
    ...
    if (agent_id.length < 3) {
      agent_id = DateTime.now().millisecondsSinceEpoch.toString();
    }
  ...
  toJson() {
    return {
      ...
      'has_receive_loan': has_receive_loan,
  ...
  static Future initTable() async {
    ...
    String sql = " CREATE TABLE IF NOT EXISTS "
        ...
        ",bank_id TEXT"
        ",bank_text TEXT"
        ",other_livestock_count TEXT"
        ",poultry_count TEXT"
        ",sheep_count TEXT"
        ",goat_count TEXT"
        ",cattle_count TEXT"
        ",bank_account_number TEXT"
        ",has_receive_loan TEXT"
        ",ready_for_upload TEXT"
        ",error_message TEXT"
        ",local_id TEXT"
        ")";
  ...
  getFullName() {
    //check if the first and last name are not empty and return the full name
  ...
  getStatus() {
    //Ready for upload
    if (ready_for_upload == "Yes") {
      return "Ready for upload";
    } else if (error_message.isNotEmpty) {
  ...
  Future<String> submitSelf() async {
    if (ready_for_upload != "Yes") {
      error_message = "Not ready for upload";
      ready_for_upload = "No";
      await save();
      return "Not ready for upload";
    }

    if (parish_id.isEmpty) {
      error_message = "Parish is required";
      ready_for_upload = "No";
      await save();
    ...
    RespondModel? resp;
    try {
      resp = RespondModel(await Utils.http_post('farmers', toJson()));
    } catch (e) {
      try {
      ...
      return "Failed because ${e.toString()}";
    }
    if (resp.code != 1) {
      try {
        error_message = resp.message.toString();
        ready_for_upload = "No";
        await save();
      } catch (e) {}
      return "Failed because ${resp.message}";
    }
    try {
      await delete();
    } catch (e) {
      ready_for_upload = "No";
      await save();
      return "Failed to delete because ${e.toString()}";
    }
    return '';
  }
}
